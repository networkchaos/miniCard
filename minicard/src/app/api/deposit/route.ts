...
export async function POST(request: NextRequest) {
  try {
    const { userAddress, tokenAddress, amount, method } = await request.json()
    ...
    // 2. Create a transaction to deposit tokens
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        message: 'Transaction created successfully. Please sign to complete deposit.',
      }
    })
  } catch (error) {
